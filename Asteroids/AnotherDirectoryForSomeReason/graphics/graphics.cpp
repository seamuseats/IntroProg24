...
{
    stbi_write_png(pngFileName.c_str(), width(), height(), 4, pixmap->pixels, 4*width());
}
...
#ifdef INCLUDE_SOUND
SoundInternal::SoundInternal(SoLoud::Soloud &player, const std::string& _filename, bool stream)
    : filename(_filename), player{player}
{
    if (stream) {
        SoLoud::WavStream* ws = new SoLoud::WavStream();
        if (!ws->load(filename.c_str())) {
            source = ws;
        }
        else {
...
void SoundInternal::stop(SoundHandle handle)
{
    player.stop(handle);
}

SoundHandle SoundInternal::play(double volume)
{
    return player.play(*source, volume);
}

bool SoundInternal::playing(SoundHandle handle)
{
    return player.isValidVoiceHandle(handle);
}
#endif
...
int FontInfo::getNvgFontFaceId(Graphics& g) const
{
    switch (face)
    {
    case mssm::FontFace::Roboto:
        return g.fontRegular;
    case mssm::FontFace::RobotoBold:
        return g.fontBold;
    case mssm::FontFace::RobotoLight:
        return g.fontLight;
    case mssm::FontFace::User1:
    case mssm::FontFace::User2:
    case mssm::FontFace::User3:
    case mssm::FontFace::User4:
    case mssm::FontFace::User5:
    case mssm::FontFace::User6:
    case mssm::FontFace::User7:
    case mssm::FontFace::User8:
    case mssm::FontFace::User9:
    case mssm::FontFace::User10:
...
void Graphics::setClip(Vec2d pos, double w, double h)
{
    nvgScissor(vg, pos.x, pos.y, w, h);
}
...
void Graphics::music(const std::string& filename, double volume)
{
...
void Graphics::setMusicVolume(double volume)
{
...
SoundHandle Graphics::play(Sound sound, double volume)
{
#ifdef INCLUDE_SOUND
...
bool Graphics::isPlaying(SoundHandle handle)
{
#ifdef INCLUDE_SOUND
...
void Graphics::text(Vec2d pos, const FontInfo& sizeAndFace, const std::string &str, Color c, HAlign hAlign, VAlign vAlign)
{
    nvgFontSize(vg, sizeAndFace.size);
    //nvgFontFace(vg, "sans");
    nvgFontFaceId(vg, sizeAndFace.getNvgFontFaceId(*this));
...
double Graphics::textWidth(double size, const string &str)
{
    float metrics[4];
...
template<typename T>
inline void Graphics::t_polygon(T points, Color border, Color fill)
{
    bool first = true;
...
template<typename T>
inline void Graphics::t_polyline(T points, Color color)
{
    bool first = true;
...
template<typename T>
inline void Graphics::t_points(T points, Color c)
{
    if (points.size() == 0) {
        return;
    }
...
void Graphics::polygon(const std::vector<Vec2d>& points, Color border, Color fill)
{
    t_polygon(points, border, fill);
}

void Graphics::polyline(const std::vector<Vec2d>& points, Color color)
{
    t_polyline(points, color);
}

void Graphics::points(const std::vector<Vec2d>& points, Color c)
{
    t_points(points, c);
}

void Graphics::polygon(const Array<Vec2d>& points, Color border, Color fill)
{
    t_polygon(points, border, fill);
}

void Graphics::polyline(const Array<Vec2d>& points, Color color)
{
    t_polyline(points, color);
}

void Graphics::points(const Array<Vec2d>& points, Color c)
{
    t_points(points, c);
}

void Graphics::polygon(std::initializer_list<Vec2d> points, Color border, Color fill)
{
    t_polygon(points, border, fill);
}

void Graphics::polyline(std::initializer_list<Vec2d> points, Color color)
{
    t_polyline(points, color);
}

void Graphics::points(std::initializer_list<Vec2d> points, Color c)
{
    t_points(points, c);
}
