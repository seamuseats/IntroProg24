...
void nvgArc2(NVGcontext* ctx, float cx, float cy, float rx, float ry, float a0, float a1, int dir)
{
    float a = 0, da = 0, hda = 0, kappa = 0;
    float dx = 0, dy = 0, x = 0, y = 0, tanx = 0, tany = 0;
    float px = 0, py = 0, ptanx = 0, ptany = 0;
...
void nvgTextBox(NVGcontext* ctx, float x, float y, float breakRowWidth, const char* string, const char* end)
{
	...
	while ((nrows = nvgTextBreakLines(ctx, string, end, breakRowWidth, rows, maxRows))) {
		for (i = 0; i < nrows; i++) {
...
void nvgTextBoxBounds(NVGcontext* ctx, float x, float y, float breakRowWidth, const char* string, const char* end, float* bounds)
{
	...
	while ((nrows = nvgTextBreakLines(ctx, string, end, breakRowWidth, rows, 200))) {
		for (i = 0; i < nrows; i++) {
			NVGtextRow* row = &rows[i];
